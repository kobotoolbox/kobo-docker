...
if [ "$IS_OPENED" == "1" ]; then
    ...
    echo ${KOBO_POSTGRES_REPLICATION_PASSWORD} | PGUSER="${PGUSER:-postgres}" PGPASSFILE=${POSTGRES_REPO//\"/}/.pgpass pg_basebackup -h ${KOBO_POSTGRES_MASTER_ENDPOINT//\"/} --port=${POSTGRES_PORT//\"/} -D ${POSTGRES_DATA_DIR//\"/} -U ${KOBO_POSTGRES_REPLICATION_USER//\"/} -v -P --xlog -c fast -W
